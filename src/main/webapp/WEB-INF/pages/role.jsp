...
<script type="text/javascript">
    $(function () {
        ...
        let roleListTemplate = $("#roleListTemplate").html();
        Mustache.parse(roleListTemplate);
        let selectedUsersTemplate = $("#selectedUsersTemplate").html();
        Mustache.parse(selectedUsersTemplate);
        let unSelectedUsersTemplate = $("#unSelectedUsersTemplate").html();
        ...
        let setting = {
            check: {
                enable: true,//显示
                chkDisabledInherit: true,//层级操作
                chkboxType: {"Y": "ps", "N": "ps"}, //auto check 父节点 子节点
                autoCheckTrigger: true//触发某个函数
            },
            data: {
                simpleData: {
                    enable: true,
        ...
        function onClickTreeNode(e, treeId, treeNode) { // 绑定单击事件
            let zTree = $.fn.zTree.getZTreeObj("roleAclTree");
            zTree.expandNode(treeNode);
        }

        function loadRoleList() {
            $.ajax({
                url: "/sys/role/queryAllRole.json",
                success: function (result) {
                    if (result.result) {
                        let rendered = Mustache.render(roleListTemplate, {roleList: result.data});
        ...
        function handleRoleSelected(roleId) {
            if (lastRoleId != -1) {
                let lastRole = $("#role_" + lastRoleId + " .dd2-content:first");
                lastRole.removeClass("btn-yellow");
                lastRole.removeClass("no-hover");
            }
            let currentRole = $("#role_" + roleId + " .dd2-content:first");
            currentRole.addClass("btn-yellow");
            currentRole.addClass("no-hover");
            lastRoleId = roleId;//最后角色选中的id
        ...
        //加载权限模块与权限点树  selectedRoleId当前角色id
        function loadRoleAcl(selectedRoleId) {
            if (selectedRoleId == -1) {
                return;
            }
            $.ajax({
                url: "/sys/core/coreTree.json",
                data: {
        ...
        function getTreeSelectedId() {
            let treeObj = $.fn.zTree.getZTreeObj("roleAclTree");
            //获得选中的节点
        ...
        function renderRoleTree(aclModuleList) {//权限模块整体树
            zTreeObj = [];//每次清空一下
            recursivePrepareTreeData(aclModuleList);
            for (let key in nodeMap) {//nodeMap 放的权限模块

                zTreeObj.push(nodeMap[key]);//权限点 加权限模块全部放在ztreeobj
            }
        ...
        function recursivePrepareTreeData(aclModuleList) {
            if (aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function (i, aclModule) {//SysAclModuleTreeDto
                    let hasChecked = false;//是否选中为false
                    if (aclModule.aclList && aclModule.aclList.length > 0) {
                        $(aclModule.aclList).each(function (i, acl) {//权限点
                            zTreeObj.push({//zTreeObj装数据
                                id: aclPrefix + acl.id,//id:a_权限点id
                                pId: modulePrefix + acl.aclModuleId,//挂在谁的下面 就是pid:m_当前权限点的权限模块id
                                name: acl.name + ((acl.type == 1) ? '(菜单)' : ''),//type 1:菜单 2：按钮  权限点的名字
                                chkDisabled: !acl.hasAcl,//设置节点的 checkbox / radio 是否禁用  根据当前用户来判断复选框是否可点
                                checked: acl.checked,//根据当前角色是否有这个权限点 来判断是否选中
                                dataId: acl.id  //待定  自定义属性
                            });
                            if (acl.checked) {//如果子权限点 有任何一个选中 就设置为true
                                hasChecked = true;
                            }
                        });
                    }
                    //循环子模块  nodeMap以权限模块id为key
                    if ((aclModule.list && aclModule.list.length > 0) ||
                        (aclModule.aclList && aclModule.aclList.length > 0)) {
                        nodeMap[modulePrefix + aclModule.id] = {//m_模块id作为key
                            id: modulePrefix + aclModule.id,
                            pId: modulePrefix + aclModule.parentId,
                            name: aclModule.name,
                            open: hasChecked //根据子节点是否有被勾选的 来判断父节点的状态
                        };
                        let tempAclModule = nodeMap[modulePrefix + aclModule.id];
                        while (hasChecked && tempAclModule) {//递归回去影响父节点
                            if (tempAclModule) {
                    ...
                    recursivePrepareTreeData(aclModule.list);
                });
            }
        }
        ...
        $(".saveRoleAcl").click(function (e) {
            ...
            $.ajax({
                url: "/sys/roleAcl/changeAcls.json",
                data: {
                    roleId: lastRoleId,
        ...
        function updateRole(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "/sys/role/insert.json" : "/sys/role/update.json",
        ...
        function loadRoleUser(selectedRoleId) {
            $.ajax({
                ...
                success: function (result) {
                    console.log(result);
                    if (result.result) {
                        let renderedSelect = Mustache.render(selectedUsersTemplate, {userList: result.data.selected});
                        let renderedUnSelect = Mustache.render(unSelectedUsersTemplate, {userList: result.data.unselected});
        ...
        $(".saveRoleUser").click(function (e) {
            ...
            $.ajax({
                url: "/sys/roleUser/changeUsers.json",
                data: {
                    roleId: lastRoleId,
